...
# ─────────────── Parse Arguments ────────────────
ARGS=$(getopt -o i:1:2:t:m:p:h --long input_dir:,r1_suffix:,r2_suffix:,threads:,mode:,platform:,help -n 'dada2_pipeline.sh' -- "$@")
[[ $? -ne 0 ]] && usage
eval set -- "$ARGS"
...
# ─────────────── Step 1: fastp ─────────────────
log "🧼 Step 1: fastp filtering and QC"
start_t=$(date +%s)
if [[ "$mode" == "se" ]]; then
    seqkit stats -j "$threads" $(ls "$input_dir"/*"$r1_suffix") > seqkit.stat.tsv
else
    seqkit stats -j "$threads" $(ls "$input_dir"/*"$r1_suffix"; ls "$input_dir"/*"$r2_suffix" ) > seqkit.stat.tsv
fi
...
if [[ "$mode" == "pe" ]]; then
  ...
  echo "$sample_list" | rush -j "$threads" -v r1="$r1_suffix",inpurt_dir="$input_dir" -c --eta --succ-cmd-file fastp.rush.finished 'fastp -i {inpurt_dir}/{1}{r1} -o 01_fastp/{1}{r1} --thread 1 --length_required 100 --n_base_limit 0 --cut_tail --qualified_quality_phred 20 --unqualified_percent_limit 20 --html /dev/null --json /dev/null &> 01_fastp/{1}.fastp.log'
fi
...
start_t=$(date +%s)
mkdir -p 02_cutadapt
primer_file="/share/cn1_fs/database/dada2_gtdb_ref/16s_primer.tsv"
...
if [[ "$mode" == "pe" ]]; then
  ...
  echo "$sample_list" | rush -j "$threads" -v r1="$r1_suffix",r2="$r2_suffix",opt="${cutadapt_opts}" -c --eta --succ-cmd-file cutadapt.rush.finished '/home/conda_envs/busco/bin/cutadapt {opt} -o 02_cutadapt/{1}{r1} -p 02_cutadapt/{1}{r2} 01_fastp/{1}{r1} 01_fastp/{1}{r2} &> 02_cutadapt/{1}.cutadapt.log'
else
  cutadapt_opts="$fr_primers --revcomp -j 1"
  echo "$sample_list" | rush -j "$threads" -v r1="$r1_suffix",opt="${cutadapt_opts}" -c --eta --succ-cmd-file cutadapt.rush.finished '/home/conda_envs/busco/bin/cutadapt {opt} -o 02_cutadapt/{1}{r1} 01_fastp/{1}{r1} &> 02_cutadapt/{1}.cutadapt.log'
fi
log "$(elapsed $start_t)"
...
log "📤 DADA2 job submitted via slurm"
log "📋 Command: $dd_cmd"
echo $dd_cmd